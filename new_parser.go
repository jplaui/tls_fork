...
func newTrafficData(filePath string, version int, cipher any) trafficData {
	return trafficData{
		filePath:          filePath,
		vers:              version,
...
// func (p *Parser) ParseServerCertificate() {
...
func (p *Parser) ReadTranscript() error {
	...
	p.transcript.Write(chTranscript)

	// fmt.Println("p.clientHello", p.clientHello)
	...
	p.serverHello = serverHello

	// marshal serverHello
	shTranscript, err := serverHello.marshal()
	if err != nil {
		log.Error().Err(err).Msg("serverHello.marshal()")
		return err
	}
	p.transcript.Write(shTranscript)
	// fmt.Printf("server hello record(without header): %x\n", serverHello.raw)

	// capture H2
	p.h2 = hex.EncodeToString(p.transcript.Sum(nil))
	// fmt.Println("p.serverHello", p.serverHello)
	// fmt.Println("H2", hex.EncodeToString(p.transcript.Sum(nil)))
...
// func (p *Parser) parseServerEncryptedExtension() (interface{}, error) {
...
// func (p *Parser) parseServerCertificate() (interface{}, error) {
// 	msg, err := p.parseHandshake()
...
// func (p *Parser) parseFinishedMsg() (interface{}, error) {
// 	msg, err := p.parseHandshake()
// 	if err != nil {
// 		return nil, errors.New("tls parser: server encrypted extension error")
// 	}
